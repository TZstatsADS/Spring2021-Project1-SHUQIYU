...
```{r 5.1.2}
anes_to_children= anes_burden %>%
  filter(!is.na(Number_of_children) & !is.na(vote))%>%
  group_by(Number_of_children,year)%>%
  count(vote)%>%
  group_by(Number_of_children,year)%>%
  mutate(
...
```
##participating due to the age of children they have

```{r 5.1.2}
anes_to_children1= anes_burden %>%
  filter(!is.na(children_under_6)&& !is.na(children_6_9) &&!is.na(children_10_13)&&
           !is.na(children_14_17)&& !is.na(vote))%>%
  group_by(vote)%>%
  count(vote)
...
ggplot(anes_to_children1, 
       aes(x=year, y=vote)) +
  geom_bar(stat="identity", colour="black")+ theme_bw()+
  theme(axis.text.x = element_text(angle = 90))+
  scale_fill_brewer(palette="Reds")+
  labs(title="How did people with differnet number of children participated \n in the election over the years?")
...
```
## 5.4 voting due to their gender
```{r }
